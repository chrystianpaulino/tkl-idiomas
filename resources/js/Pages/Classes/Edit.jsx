...
export default function ClassesEdit({ turmaClass, professors }) {
    const { data, setData, put, processing, errors } = useForm({
        name: turmaClass.name,
        professor_id: turmaClass.professor_id,
        description: turmaClass.description ?? '',
    });

    function submit(e) {
        e.preventDefault();
        put(`/classes/${turmaClass.id}`);
    }

    return (
        <AppLayout title="Editar Turma">
            <Head title="Editar Turma" />
            <PageHeader title="Editar Turma" subtitle={`Editando: ${turmaClass.name}`}>
